...
int XKLIMAMREncodeMode[] = {4750, 5150, 5900, 6700, 7400, 7950, 10200, 12200}; // amr 编码方式
...
{
	XKLIMRIFFHEADER riff;
	XKLIMFMTBLOCK fmt;
	XKLIMXCHUNKHEADER chunk;
	XKLIMWAVEFORMATX wfx;
	int bDataBlock = 0;
	
	// 1. 读RIFF头
	fread(&riff, 1, sizeof(XKLIMRIFFHEADER), fpwave);
	
	// 2. 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
	fread(&chunk, 1, sizeof(XKLIMXCHUNKHEADER), fpwave);
	if ( chunk.nChunkSize>16 )
	{
		fread(&wfx, 1, sizeof(XKLIMWAVEFORMATX), fpwave);
	}
	else
	{
		memcpy(fmt.chFmtID, chunk.chChunkID, 4);
		fmt.nFmtSize = chunk.nChunkSize;
		fread(&fmt.wf, 1, sizeof(XKLIMWAVEFORMAT), fpwave);
	}
...
//	unsigned short ush1=0, ush2=0, ush=0;
	...
	// 原始PCM音频帧数据
	unsigned char  pcmFrame_8b1[XKLIM_PCM_FRAME_SIZE];
	unsigned char  pcmFrame_8b2[XKLIM_PCM_FRAME_SIZE<<1];
	unsigned short pcmFrame_16b1[XKLIM_PCM_FRAME_SIZE];
	unsigned short pcmFrame_16b2[XKLIM_PCM_FRAME_SIZE<<1];
	
	if (nBitsPerSample==8 && nChannels==1)
	{

		nRead = fread(pcmFrame_8b1, (nBitsPerSample/8), XKLIM_PCM_FRAME_SIZE*nChannels, fpwave);

		for(x=0; x<XKLIM_PCM_FRAME_SIZE; x++)
		{
		...
		{
			nRead = fread(pcmFrame_8b2, (nBitsPerSample/8), XKLIM_PCM_FRAME_SIZE*nChannels, fpwave);
			for( x=0, y=0; y<XKLIM_PCM_FRAME_SIZE; y++,x+=2 )
			{
				// 1 - 取两个声道之左声道
			...
			{
				nRead = fread(pcmFrame_16b1, (nBitsPerSample/8), XKLIM_PCM_FRAME_SIZE*nChannels, fpwave);
				for(x=0; x<XKLIM_PCM_FRAME_SIZE; x++)
				{
				...
				{
					nRead = fread(pcmFrame_16b2, (nBitsPerSample/8), XKLIM_PCM_FRAME_SIZE*nChannels, fpwave);
					for( x=0, y=0; y<XKLIM_PCM_FRAME_SIZE; y++,x+=2 )
					{
	...
	if (nRead<XKLIM_PCM_FRAME_SIZE*nChannels) return 0;
...
int XKLIM_EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
	...
	/* input speech vector */
	short speech[XKLIM_PCM_FRAME_SIZE];
	...
	/* bitstream filetype */
	unsigned char amrFrame[XKLIM_MAX_AMR_FRAME_SIZE];
	...
	bytes = fwrite(XKLIM_AMR_MAGIC_NUMBER, sizeof(char), strlen(XKLIM_AMR_MAGIC_NUMBER), fpamr);
...
void WriteWAVEFileHeader(FILE* fpwave, int nFrame)
{
	...
	+ sizeof(XKLIMXCHUNKHEADER)               // fmt
	+ sizeof(XKLIMWAVEFORMATX)           // WAVEFORMATX
	+ sizeof(XKLIMXCHUNKHEADER)               // DATA
	+ nFrame*160*sizeof(short);    //
	strcpy(tag, "WAVE");
	memcpy(riff.chRiffFormat, tag, 4);
	fwrite(&riff, 1, sizeof(XKLIMRIFFHEADER), fpwave);
	...
	chunk.nChunkSize = sizeof(XKLIMWAVEFORMATX);
	fwrite(&chunk, 1, sizeof(XKLIMXCHUNKHEADER), fpwave);
	memset(&wfx, 0, sizeof(XKLIMWAVEFORMATX));
	wfx.nFormatTag = 1;
	wfx.nChannels = 1; // 单声道
	...
	fwrite(&wfx, 1, sizeof(XKLIMWAVEFORMATX), fpwave);
...
int caclAMRFrameSize(unsigned char frameHeader)
{
	...
	mode = XKLIMAMREncodeMode[temp1];
	
	// 计算amr音频数据帧大小
	// 原理: amr 一帧对应20ms，那么一秒有50帧的音频数据
	temp2 = myround((double)(((double)mode / (double)XKLIM_AMR_FRAME_COUNT_PER_SECOND) / (double)8));
...
int XKLIM_DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
	...
	unsigned char amrFrame[XKLIM_MAX_AMR_FRAME_SIZE];
	short pcmFrame[XKLIM_PCM_FRAME_SIZE];
...
//	fpamr = fopen([path cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	...
	// 检查amr文件头
	fread(magic, sizeof(char), strlen(XKLIM_AMR_MAGIC_NUMBER), fpamr);
	if (strncmp(magic, XKLIM_AMR_MAGIC_NUMBER, strlen(XKLIM_AMR_MAGIC_NUMBER)))
	{
		fclose(fpamr);
		return 0;
	}
...
//	fpwave = fopen([docFilePath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	...
	nFrameCount++;
	fwrite(pcmFrame, sizeof(short), XKLIM_PCM_FRAME_SIZE, fpwave);
...
int ReadPCMFrameData(short speech[], char* fpwave, int nChannels, int nBitsPerSample)
{
    ...
    nRead = (nBitsPerSample/8) * XKLIM_PCM_FRAME_SIZE*nChannels;
    if (nBitsPerSample==8 && nChannels==1)
    {
        //nRead = fread(pcmFrame_8b1, (nBitsPerSample/8), XKLIM_PCM_FRAME_SIZE*nChannels, fpwave);
...
// bps = 8 --> 8位 unsigned char
//       16 --> 16位 unsigned short
NSData* XKLIM_EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample)
{
    char* oldBuf = data;
    /* input speech vector */
    short speech[XKLIM_PCM_FRAME_SIZE];
    ...
    /* write magic number to indicate single channel AMR file storage format */
    //bytes = fwrite(XKLIM_AMR_MAGIC_NUMBER, sizeof(char), strlen(XKLIM_AMR_MAGIC_NUMBER), fpamr);
    [out appendBytes:XKLIM_AMR_MAGIC_NUMBER length:strlen(XKLIM_AMR_MAGIC_NUMBER)];
...
//    NSArray *paths               = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *documentPath       = [paths objectAtIndex:0];
//    NSString *wavFile        = [documentPath stringByAppendingPathComponent:[NSString stringWithFormat:@"tempRecord.amr"]];
...
NSData* XKLIM_EncodeWAVEToAMR(NSData* data, int nChannels, int nBitsPerSample)
{
    ...
    return XKLIM_EncodePCMToAMR(buf,maxLen- nPos,nChannels,nBitsPerSample);
}


#pragma mark - data decode

NSData * android3GP(NSData *data) {
...
void WriteWAVEHeader(NSMutableData* fpwave, int nFrame)
{
    ...
    [fpwave appendBytes:&riff length:sizeof(XKLIMRIFFHEADER)];
    ...
    [fpwave appendBytes:&chunk length:sizeof(XKLIMXCHUNKHEADER)];
    memset(&wfx, 0, sizeof(XKLIMWAVEFORMATX));
    wfx.nFormatTag = 1;
    wfx.nChannels = 1; // 单声道
    wfx.nSamplesPerSec = 8000; // 8khz
    ...
    [fpwave appendBytes:&wfx length:sizeof(XKLIMWAVEFORMATX)];
...
NSData* XKLIM_DecodeAMRToWAVE(NSData* data) {
    ...
    //有可能是android 3gp格式
    if (strncmp(rfile, XKLIM_AMR_MAGIC_NUMBER, strlen(XKLIM_AMR_MAGIC_NUMBER)))
    {
        data = android3GP(data);
    }
    ...
    pos += strlen(XKLIM_AMR_MAGIC_NUMBER);
    // 创建并初始化WAVE文件
    ...
    [fpwave appendBytes:pcmFrame length:XKLIM_PCM_FRAME_SIZE*sizeof(short)];
